...
Data types, data structures, maths operators or control flow structures 
...
In a different context, the Boolean algebra, the values of the variables are the truth values: true, false, instead of numbers.
...
A logical operator on mathematical statements is used to combine one or more statements to create a new one. 
...
with a different bunch of operators in order to work with them. 
        ...
        \lstpython
        \renewcommand{\home}{./Python/sources/Foundations/data_type} 
        \listingsp{\home/data_type.py}{def Operators}{1 in Primes}{data_type.py}
...
In programming we have seen that the variables are built by 
a memory storage location tagged with a symbolic name.
While the name is just a tag to identify the variable,
the content of the memory location is the data itself. 
