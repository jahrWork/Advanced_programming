...
%vectors
The term vector in mathematics has different formal definitions depending on the context.
Vector is usually used for tuples, which means, finite ordered lists (sequences) of elements. 
An $n-$tuple for example is an ordered list of $n$ elements, where $n$ is a non-negative integer.
However, historically the term vector was used for geometric vectors, objects that has magnitude and direction.
Finally, in pure mathematics a vector is any element of a vector space. 

Indeed, vectors seen as tuples of elements in a field $\mathbb{F}$ together with the common addition of vectors and the multiplication by an scalar in the field $\mathbb{F}$ build a vector space. 
...
A tensor is usually denoted by a symbolic name followed by a collection of subscripts where 
the number of subscripts attached defines the rank of the tensor. 
...
%notacion de cada uno
%con parentesis
%con corchetes cuadrados
%con llaves
 

en programacion como los convertimos
definimos data structures
