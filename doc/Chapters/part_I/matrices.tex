...
\vspace{-.5cm}
\lstpython
\listings{\home/Matrix_operations.py}{def ElementWise_operation_examples}
...
Static and dynamic allocations have been already treated. 
...
Let's review some properties associated to each memory region:
\begin{itemize}[noitemsep]
    \item \textbf{Static:} it is usually located in the low end of the memory reserved for the application. 
    The compiler creates a list of variables to be allocated during the compilation and gives them a fixed address.
    
    The whole program can access these variables at any time in a quick way
    but it requires knowing the amount of memory needed at compile-time.
    
    \item \textbf{Heap:} the program requests a specific amount of memory (depending on the purpose) to the system in run-time.
    ...
    Usually, the amount of heap space is bigger than the stack (although slower), but not unlimited. 
    Hence, if no deallocation is performed, a memory leak can happen in the sense that the memory is still reserved with no useful data.
    Concepts like virtual memory and swap play an important role. 
    Notice that the program decides the amount of memory needed and the programmer has control over this memory in several occasions, 
    
    \item \textbf{Stack:} it is composed by a limited amount of memory and a pointer that marks how full it is. 
    With this pointer, the routines know where to store local variables. 
    This memory space is filled from the top to the bottom so any time that a subroutine or function (let's say \texttt{function A}) needs from temporary storage this space is used, the pointer is decremented and the stack space is reduced. If this function calls a nested \texttt{function B}, then its local variables are stored below variables of \texttt{A} and the pointer is decremented once again. Once the \texttt{routine B} finishes its operations, the pointer is incremented again below the spaced filled by \texttt{routine A} so that memory is available for the next routine. This structure is called LIFO; 'Last-In,-First-Out'.  
