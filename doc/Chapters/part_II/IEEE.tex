...
the computer counts with a finite amount of memory to store the binary value.

\begin{enumerate}
    \item \textbf{Fixed-point representation} fixes the position of the binary point that separates 
    the integer part from the decimal part. Then, the bits reserved for both parts are prefixed. 
    
    \item \textbf{Floating-point representation} stores the value of the mantissa in some bits and the value of the exponent in different bits.
\end{enumerate}
...
The numbers are encoded similarly to binary standard scientific notation. 
...
which is $31.875$. As you may have seen, no negative values are encoded here. 
...
let's consider now a mini-float of same size, 8 bits.
In this case 4 bits are reserved for the exponent (1 for its sign and 3 for its value) and 4 bits for the mantissa. 
...
$$
Notice that $b_0 = 1$ considers the implicit 1 by adding $1\times2^0 = 1$ to the fractional mantissa. %The rest of terms go through the mantissa bits. 
...
The standard IEEE 754 reserves some combinations of binary digits for special situations: \texttt{NaN}, $\pm\infty$ or denormalised numbers (see Table \ref{tab:SpecialValues}). In addition, take into account that there are two numbers equal to $0$: $\pm 0$.
...
The denormalised numbers are used when \textbf{underflow} occurs, which means, a value in the gap that exits between the smallest normalised number representable (\texttt{tiny}) and the same negative value. With the denormalised or subnormal numbers a gradual underflow is achieved so numbers too small to be represented (otherwise replaced by 0) are gradually decreased. From the representation point of view, the number is not treated with an implicit 1 before the binary point in the mantissa, but with a zero. Hence, the range of the mantissa is $\left[ 0, 1\right)$.
...
by underscoring the constant with its kind type. However, this is not the common way to do it, 
consider not declaring the kind for any constants neither so it automatically adopts the default real kind value.
...
10.e0
\end{verbatim}
First of all, notice that \texttt{10} can not be used since it would be treated as an integer. Secondly, remember that the kind of these constants \textbf{is not necessarily single precision}, default precision is being used with scientific notation so the \texttt{default real kind} option of the compiler rules the precision of these constants.
...
Use the following subroutine to reconstruct a real number through its internal bits representation: 
